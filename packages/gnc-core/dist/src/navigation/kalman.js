export class KalmanFilter3D {
    ...
    H; // 6x6
    Q_base; // scaled per dt
    R; // 6x6
    constructor(initial, covariances, noise) {
        const { r, v } = initial;
    ...
    update(z) {
        ...
        // x = x + K y
        this.x = addVec6(this.x, matVec6(K, y));
        // P = (I - K H) P
        const I = identity(6);
        const KH = matMul(K, H);
...
function scaleQ(Qbase, dt) {
    // Scale process noise approximately with dt
    return scale(Qbase, Math.max(dt, 1e-6));
}
function identity(n) { return diag(Array(n).fill(1)); }
function zeros(n) { return Array.from({ length: n }, () => Array(n).fill(0)); }
function diag(vals) { return vals.map((v, i) => vals.map((_, j) => (i === j ? v : 0))); }
function scale(M, s) { return M.map(row => row.map(x => x * s)); }
...
function matVec6(A, x) {
    const y = matVec(A, x);
    return [y[0], y[1], y[2], y[3], y[4], y[5]];
}
function addVec6(a, b) { return [a[0] + b[0], a[1] + b[1], a[2] + b[2], a[3] + b[3], a[4] + b[4], a[5] + b[5]]; }
...
function swapRows(M, i, j) { if (i !== j) {
    const t = M[i];
    M[i] = M[j];
    M[j] = t;
} }
