...
export class PIDController {
  ...
  private previous_error: number = 0
  private dt: number
...
export class AttitudeControlSystem {
  private pitch_controller: PIDController
  private yaw_controller: PIDController
  private roll_controller: PIDController
  private max_gimbal_angle: number = 8 * Math.PI / 180 // 8 degrees max gimbal
   ...
   */
  computeTVC(
    commanded_attitude: { pitch: number, yaw: number, roll: number },
    current_attitude: { pitch: number, yaw: number, roll: number }
  ): { pitch_gimbal: number, yaw_gimbal: number } {

    const pitch_command = this.pitch_controller.update(
      commanded_attitude.pitch,
      current_attitude.pitch
    )

    const yaw_command = this.yaw_controller.update(
      commanded_attitude.yaw,
      current_attitude.yaw
    )

    // Limit gimbal angles
    const pitch_gimbal = Math.max(-this.max_gimbal_angle,
      Math.min(this.max_gimbal_angle, pitch_command))
    const yaw_gimbal = Math.max(-this.max_gimbal_angle,
      Math.min(this.max_gimbal_angle, yaw_command))
   ...
   */
  computeRCS(commanded_roll: number, current_roll: number): number {
...
export class LaunchAutopilot {
  private attitude_control: AttitudeControlSystem
  private throttle_controller: PIDController
  private target_acceleration: number = 20 // m/s² target acceleration
   ...
   */
  update(state: LaunchState): LaunchControlCommands {
    // Attitude control
  ...
  private computeAttitudeControl(state: LaunchState): AttitudeCommands {
  ...
  private computeThrottleControl(state: LaunchState): number {
  ...
  private computeEngineControl(state: LaunchState): EngineCommands {
  ...
  private computeRCSControl(state: LaunchState): RCSCommands {
  ...
  private computeInjectionPressure(state: LaunchState): number {
...
export class ReactionControlSystem {
  private thrusters: RCSThruster[]
  private moment_arm: number = 2.0 // m from CG to thrusters
   ...
   */
  computeThrusterCommands(desired_torque: Vec3): number[] {
...
export class GNCSystem {
  private autopilot: LaunchAutopilot
  private rcs: ReactionControlSystem
  private control_active: boolean = true
   ...
   */
  update(state: LaunchState): GNCCommands {
  ...
  private shouldSeparateStage(state: LaunchState): boolean {
    return state.phase === LaunchPhase.STAGE1_SEPARATION
  }

  private shouldJettisonFairing(state: LaunchState): boolean {
    return state.phase === LaunchPhase.FAIRING_JETTISON
  }

  private getInactiveCommands(): GNCCommands {
    return {
      main_engine: {
        throttle: 0,
        gimbal_pitch: 0,
 ...
 */
export interface LaunchControlCommands {
  attitude: AttitudeCommands
  throttle: number
  engines: EngineCommands
  rcs: RCSCommands
}

export interface AttitudeCommands {
  pitch_gimbal: number // rad
  yaw_gimbal: number   // rad
  roll_command: number // rad/s
}

export interface EngineCommands {
  ignition_command: boolean
  shutdown_command: boolean
  mixture_ratio: number
  injection_pressure: number // bar
}

export interface RCSCommands {
  active: boolean
  thrust_commands: Vec3 // N⋅m torque commands
}

export interface GNCCommands {
  main_engine: {
    throttle: number
    gimbal_pitch: number
    gimbal_yaw: number
  ...
  rcs: {
    active: boolean
    thruster_commands: number[]
  }
  vehicle: {
    stage_separation: boolean
    fairing_jettison: boolean
  }
}

interface RCSThruster {
  position: Vec3    // m position relative to CG
  direction: Vec3   // unit vector thrust direction
  thrust: number    // N maximum thrust
}
...
export interface ControlMetrics {
  attitude_error: {
    pitch: number    // rad RMS error
    yaw: number      // rad RMS error
    roll: number     // rad RMS error
  }
  guidance_error: {
    cross_track: number   // m cross-track error
    down_track: number    // m down-track error
    velocity: number      // m/s velocity error
  }
  propellant_efficiency: {
    gravity_losses: number    // m/s delta-V lost to gravity
    drag_losses: number       // m/s delta-V lost to drag
    steering_losses: number   // m/s delta-V lost to steering
  }
}
...
export class MissionAnalyzer {
   ...
   */
  static analyzeLaunchPerformance(trajectory: LaunchState[]): ControlMetrics {
  ...
  private static computeGravityLosses(trajectory: LaunchState[]): number {
  ...
  private static computeDragLosses(trajectory: LaunchState[]): number {
    // Drag losses = ∫ (D/m) dt
    let losses = 0
    for (let i = 1; i < trajectory.length; i++) {
      const dt = trajectory[i].mission_time - trajectory[i-1].mission_time
  ...
  private static computeSteeringLosses(trajectory: LaunchState[]): number {
