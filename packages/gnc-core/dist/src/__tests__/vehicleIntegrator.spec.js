...
import { VehicleIntegrator } from '../launch/integration';
...
describe('VehicleIntegrator – initialisation', () => {
    let integrator;
    beforeEach(() => {
        integrator = new VehicleIntegrator([SRB_STAGE, CORE_STAGE, ICPS_STAGE], STAGING_EVENTS, PAYLOAD_MASS);
    });
    ...
    it('initial total mass equals sum of all stage masses + payload', () => {
        const state = integrator.getState();
        const expected = (SRB_STAGE.dryMass + SRB_STAGE.propMass) +
...
describe('VehicleIntegrator – getState immutability', () => {
    it('getState returns a copy, not a direct reference', () => {
        const integrator = new VehicleIntegrator([SRB_STAGE, CORE_STAGE], [], PAYLOAD_MASS);
...
describe('VehicleIntegrator – thrust-to-weight ratio', () => {
    it('T/W > 1 at liftoff (required for ascent)', () => {
        const integrator = new VehicleIntegrator([SRB_STAGE, CORE_STAGE, ICPS_STAGE], STAGING_EVENTS, PAYLOAD_MASS);
