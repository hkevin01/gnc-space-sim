...
describe('KalmanFilter3D – initialisation', () => {
    it('constructs without error', () => {
        expect(() => new KalmanFilter3D({ r: [7000e3, 0, 0], v: [0, 7500, 0] }, { posVar: 1e4, velVar: 10 }, { processPosStd: 2, processVelStd: 0.5, measPosStd: 20, measVelStd: 1 })).not.toThrow();
    });
    it('initial x[0] matches provided position r[0]', () => {
        const kf = new KalmanFilter3D({ r: [7000e3, 0, 0], v: [0, 7500, 0] }, { posVar: 1e4, velVar: 10 }, { processPosStd: 2, processVelStd: 0.5, measPosStd: 20, measVelStd: 1 });
        expect(kf.x[0]).toBeCloseTo(7000e3, -5);
        expect(kf.x[4]).toBeCloseTo(7500, -3);
    });
    it('initial covariance matrix P is 6×6', () => {
        const kf = new KalmanFilter3D({ r: [7000e3, 0, 0], v: [0, 7500, 0] }, { posVar: 1e4, velVar: 10 }, {});
...
describe('KalmanFilter3D – predict / update cycle', () => {
    let kf;
    beforeEach(() => {
        kf = new KalmanFilter3D({ r: [7000e3, 0, 0], v: [0, 7500, 0] }, { posVar: 1e4, velVar: 10 }, { processPosStd: 2, processVelStd: 0.5, measPosStd: 20, measVelStd: 1 });
    });
