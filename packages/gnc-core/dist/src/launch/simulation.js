...
function thrustVector(thrustMag, pitch, yaw) {
    ...
    const dir = [cx * cy, cx * sy, sx];
    return scale(unit(dir), thrustMag);
}
function getActiveStage(vehicle, t) {
...
export function integrateLaunchTrajectory(prev, vehicle, guidance, dt) {
    ...
    const v = add(prev.v, scale(accel, dt));
    const r = add(prev.r, scale(v, dt));
    // Recompute scalars
...
export function initializeLaunchState(state, vehicle) {
