...
export interface SLSStage {
  ...
  parallel?: boolean; // for SRBs
}

export interface SLSVehicleConfig {
  name: string;
  stages: SLSStage[];
  payload: {
    mass: number; // kg
  ...
  events: SLSMissionEvent[];
}

export interface SLSMissionEvent {
  name: string;
  timeFromLiftoff: number; // s
  condition?: 'burnout' | 'altitude' | 'velocity';
  conditionValue?: number;
  mass?: number; // mass to jettison
  description: string;
}
 ...
 */
export const SLSBlock1: SLSVehicleConfig = {
 ...
 */
export function calculateMassFlow(thrust: number, isp: number): number {
 ...
 */
export function getCurrentVehicleMass(config: SLSVehicleConfig, timeFromLiftoff: number): number {
 ...
 */
export function getCurrentThrust(config: SLSVehicleConfig, timeFromLiftoff: number, altitude: number): number {
  let totalThrust = 0;
