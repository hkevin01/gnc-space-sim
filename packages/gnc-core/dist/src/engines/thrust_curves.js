 ...
 */
export function interpolateThrust(time, profile) {
    // Clamp time to profile duration
    if (time <= 0)
        return profile.points[0]?.thrust * profile.maxThrust || 0;
 ...
 */
export const SLSSRBThrustProfile = {
 ...
 */
export const LiquidEngineProfile = {
 ...
 */
export const UpperStageProfile = {
 ...
 */
export function calculateTotalImpulse(profile) {
    let totalImpulse = 0;
 ...
 */
export function calculateAverageIsp(profile, propellantMass) {
 ...
 */
export function createThrustProfile(maxThrust, burnTime, thrustProfile = 'steady') {
    const points = [
        { time: 0, thrust: 0.0 }
    ];
    switch (thrustProfile) {
        case 'steady':
            points.push({ time: burnTime * 0.05, thrust: 1.0 }, { time: burnTime * 0.95, thrust: 1.0 }, { time: burnTime, thrust: 0.0 });
            break;
        case 'progressive':
            points.push({ time: burnTime * 0.1, thrust: 0.7 }, { time: burnTime * 0.5, thrust: 0.9 }, { time: burnTime * 0.9, thrust: 1.0 }, { time: burnTime, thrust: 0.0 });
            break;
        case 'regressive':
            points.push({ time: burnTime * 0.1, thrust: 1.0 }, { time: burnTime * 0.5, thrust: 0.9 }, { time: burnTime * 0.9, thrust: 0.7 }, { time: burnTime, thrust: 0.0 });
            break;
    }
    return {
 ...
 */
export function validateThrustProfile(profile) {
    const errors = [];
