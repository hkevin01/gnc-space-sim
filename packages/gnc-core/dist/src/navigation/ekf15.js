...
export class EKF15Navigation {
    ...
    accelBiasRwStd;
    gyroBiasRwStd;
    constructor(initial, covDiag, options) {
        this.x = [
    ...
    predict(imu, dt) {
        ...
        const r = [this.x[0], this.x[1], this.x[2]];
        const v = [this.x[3], this.x[4], this.x[5]];
        const euler = [this.x[6], this.x[7], this.x[8]];
        const ba = [this.x[9], this.x[10], this.x[11]];
        const bg = [this.x[12], this.x[13], this.x[14]];
        const fBody = [
        ...
        const aNav = [fNav[0] + gNav[0], fNav[1] + gNav[1], fNav[2] + gNav[2]];
...
function gravityTwoBody(r, mu) {
    const r2 = r[0] * r[0] + r[1] * r[1] + r[2] * r[2];
    const rMag = Math.sqrt(r2);
    if (!Number.isFinite(rMag) || rMag < 1)
        return [0, 0, 0];
    const k = -mu / (r2 * rMag);
    return [k * r[0], k * r[1], k * r[2]];
}
function rotationMatrixBodyToNav(roll, pitch, yaw) {
...
function attitudeAccelJacobianNumerical(euler, fBody) {
    const eps = 1e-6;
    const J = zeros(3, 3);
    for (let j = 0; j < 3; j++) {
        const ep = [euler[0], euler[1], euler[2]];
        const em = [euler[0], euler[1], euler[2]];
        ep[j] += eps;
        em[j] -= eps;
