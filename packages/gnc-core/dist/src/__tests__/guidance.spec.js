...
describe('GravityTurnGuidance', () => {
    ...
    it('throttle reduces during max-Q phase (12-100 km altitude)', () => {
        const { throttle: throttleAtMaxQ } = guidance.computeGuidance(makeState({ altitude: 13000, velocity_magnitude: 1500 }));
        const { throttle: throttleAbove } = guidance.computeGuidance(makeState({ altitude: 120000, velocity_magnitude: 1500 }));
