 ...
 */
export function propagateTwoBody(state, dt, steps, mu, mode = 'rk4') {
    if (mode === 'euler') {
        let s = state;
 ...
 */
export function rk4Propagate(state, dt, steps, mu) {
    let s = state;
    for (let i = 0; i < steps; i++)
        s = rk4Step(s, dt, mu);
    return s;
 ...
 */
export function rk4Trajectory(state, dt, steps, mu) {
    const traj = [state];
    let s = state;
    for (let i = 0; i < steps; i++) {
        s = rk4Step(s, dt, mu);
 ...
 */
export function orbitalEnergy(state, mu) {
 ...
 * a = -μ / (2ε)
 */
export function semiMajorAxis(state, mu) {
