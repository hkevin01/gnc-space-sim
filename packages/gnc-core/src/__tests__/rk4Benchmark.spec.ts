...
function circularReference(r: number, t: number, mu: number): [number, number, number, number, number, number] {
...
describe('RK4 vs Euler benchmark-style accuracy and energy tests', () => {
  it('RK4 has lower endpoint position error than Euler over one orbit', () => {
    ...
    const s0: [number, number, number, number, number, number] = [r, 0, 0, 0, v, 0]
