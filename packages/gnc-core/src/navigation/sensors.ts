...
export interface IMUMeasurement {
  acceleration: Vec3    // m/s² (specific force)
  angular_velocity: Vec3 // rad/s
  timestamp: number     // s
  bias: {
    accel_bias: Vec3    // m/s² accelerometer bias
    gyro_bias: Vec3     // rad/s gyroscope bias
  }
  noise: {
    accel_noise: number // m/s² 1-sigma noise
    gyro_noise: number  // rad/s 1-sigma noise
  }
}
...
export interface GPSMeasurement {
  ...
  accuracy: {
    position: number    // m 1-sigma position accuracy
    velocity: number    // m/s 1-sigma velocity accuracy
  }
  available: boolean   // GPS signal availability
}

/**
 * Radar Altimeter Model
 */
export interface RadarAltimeter {
  altitude: number     // m altitude above terrain
  timestamp: number    // s
  accuracy: number     // m 1-sigma accuracy
  max_range: number    // m maximum measurement range
  available: boolean   // Signal return available
}

/**
 * Navigation State (Extended Kalman Filter State)
 */
export interface NavigationState {
  position: Vec3       // m ECEF position
  velocity: Vec3       // m/s ECEF velocity
  attitude: {
    roll: number       // rad
    pitch: number      // rad
  ...
  biases: {
    accel_bias: Vec3   // m/s² IMU accelerometer bias
    gyro_bias: Vec3    // rad/s IMU gyroscope bias
  }
  covariance: number[][] // State covariance matrix
  timestamp: number    // s
}
...
export class CoordinateTransforms {
   ...
   */
  static ecefToEnuMatrix(lat: number, lon: number): number[][] {
    const sinLat = Math.sin(lat)
    const cosLat = Math.cos(lat)
    const sinLon = Math.sin(lon)
    const cosLon = Math.cos(lon)
...
export class LaunchNavigationFilter {
  private state: number[] // 15-element state vector
  private covariance: number[][] // 15x15 covariance matrix
  private process_noise: number[][] // Process noise matrix
  private dt: number // Integration time step

  constructor(initial_state: NavigationState, dt: number = 0.1) {
    this.dt = dt
   ...
   * P_k = F * P_{k-1} * F^T + Q
   */
  predict(imu: IMUMeasurement): void {
   ...
   * P = (I - K * H) * P
   */
  update(gps: GPSMeasurement): void {
  ...
  private initializeProcessNoise(): number[][] {
  ...
  private computeStateTransition(): number[][] {
  ...
  private computeControlMatrix(): number[][] {
  ...
  private computeMeasurementMatrix(): number[][] {
  ...
  private computeMeasurementNoise(gps: GPSMeasurement): number[][] {
...
export class SensorSimulator {
  ...
  static simulateIMU(true_state: LaunchState, _dt: number): IMUMeasurement {
    ...
    const accel_bias: Vec3 = [0.05, -0.02, 0.03]
    const gyro_bias: Vec3 = [0.001, 0.0005, -0.0008]
   ...
   */
  static simulateGPS(true_state: LaunchState): GPSMeasurement {
    ...
    const pos_noise: Vec3 = [
      (Math.random() - 0.5) * 2 * pos_accuracy,
      (Math.random() - 0.5) * 2 * pos_accuracy,
      (Math.random() - 0.5) * 2 * pos_accuracy
    ]

    const vel_noise: Vec3 = [
