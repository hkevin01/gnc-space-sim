...
describe('bench: dijkstra vs enhancedSSSP', () => {
  it('compares runtimes (soft check)', () => {
    ...
    const t0 = Date.now()
    const r1 = dijkstra(nodes, edges, 0)
    const dt1 = Date.now() - t0

    const t2 = Date.now()
    const r2 = enhancedSSSP(nodes, edges, 0)
    const dt2 = Date.now() - t2

    const t3 = Date.now()
    const r3 = enhancedSSSP(nodes, edges, 0)
    ...
    if (dt3 > dt1 * 10) throw new Error(`enhancedSSSP repeat too slow (${dt3}ms vs ${dt1}ms)`)
  }, 60000)
})
