...
describe('enhancedSSSP perf', () => {
  it('runs on a medium sparse graph', () => {
    ...
    const { dist } = enhancedSSSP(nodes as any, edges as any, 0)
    const dt = Date.now() - start
    let reachable = 0
    for (let i = 0; i < n; i++) if (isFinite(dist[i])) reachable++
    if (reachable === 0) throw new Error('no reachable nodes')
    if (dt > 10000) throw new Error(`enhancedSSSP too slow: ${dt}ms`)
  }, 20000)
})
